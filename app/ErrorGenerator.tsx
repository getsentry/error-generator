    ...
    for (let i = 0; i < repeats; i++) {
      try {
        ...
        toast({
          ...
          description:
            data.message || `${numErrors} errors with ${eventsPerError} events each sent to Sentry`,
          status: 'success',
          duration: 2000,
          isClosable: true,
        });
        ...
        <VStack spacing={3} align="stretch">
          <HStack spacing={4}>
            ...
            <FormControl flex={1}>
              ...
              <NumberInput min={1} value={repeatCount} onChange={(value) => setRepeatCount(value)}>
                <NumberInputField />
                <NumberInputStepper>
                  <NumberIncrementStepper />
                  <NumberDecrementStepper />
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              {intervalMode ? (
                <>
                  Warning: This will generate errors every {intervalFrequency} seconds, repeated{' '}
                  {repeatCount} times. This action will generate real errors and use up your Sentry
                  quota significantly. This may result in substantial additional costs depending on
                  your Sentry plan. Are you sure you want to proceed?
                </>
              ) : (
                <>
                  Warning: This action will generate real errors and use up your Sentry quota. This
                  may result in additional costs depending on your Sentry plan. Are you sure you
                  want to proceed?
                </>
              )}
            </AlertDialogBody>
